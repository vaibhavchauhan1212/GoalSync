...
export default function App() {
  ...
  // 📑 To capture the true live database lifecycle state!
  const [submissionStatus, setSubmissionStatus] = useState("Pending L1 Review");

  // Fetch the latest validated goals dynamically from your MongoDB Atlas Cluster
  useEffect(() => {
  ...
  // Submit execution to talk to our node server
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f1f5f9', margin: 0, fontFamily: 'system-ui, sans-serif' }}>
      ...
      {!isAuthenticated ? (
        ...
        <>
          {/* 🔐 Production Guardrail: Switcher only visible if logged in as Admin for testing */}
            ...
            {currentRole === 'Manager' && (
              <div style={{ background: '#fff', padding: '30px', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                <div style={{ borderBottom: '2px solid #e2e8f0', paddingBottom: '15px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ 
                    background: submissionStatus === 'Approved' ? '#dcfce7' : '#fef3c7', 
                    color: submissionStatus === 'Approved' ? '#16a34a' : '#d97706', 
                    padding: '6px 12px', 
                    borderRadius: '6px', 
                    fontWeight: 'bold', 
                    fontSize: '14px' 
                ...
                <div style={{ display: 'flex', gap: '15px', justifyContent: 'flex-end' }}>
                  <button onClick={() => alert("Goal sheet sent back to employee for revision.")} style={{ padding: '10px 20px', background: '#fff', border: '1px solid #cbd5e1', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', color: '#64748b' }}>
                  ...
                  <button 
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const result = await response.json();
                        if (result.status === "success") {
                          alert("🛡️ Milestone Secured: Goal sheet status has been permanently updated to APPROVED in MongoDB cloud!");
